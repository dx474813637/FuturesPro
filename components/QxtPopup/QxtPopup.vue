		...
		<PopupNormal v-bind="$attrs" @open="open">
			<template #header>
				<view class="header" >
					...
					<view class="u-flex u-flex-items-center"  v-if="!qxt_loading"> 
						...
						<template v-if="keyword">
							<template v-if="searchRes.length > 0">
								<view class="searchRes-content u-p-20 u-flex u-flex-wrap u-flex-items-center">
									...
									>
										<text>{{ele.product.slice(0, ele.pp[0])}}</text>
										<text class="u-error">{{ele.product.slice(ele.pp[0], ele.pp[0]+ele.pp[1])}}</text>
										<text>{{ele.product.slice(ele.pp[0]+ele.pp[1])}}</text>
							...
							<view
								...
								<view class="item-card u-p-10 u-p-l-20 u-p-r-20 bg-white u-m-b-20 u-radius-10">
									...
									>
										...
										></i>
										<text class="u-p-l-10">{{item.exchange}}</text>
										<text class="u-p-l-10":style="{
											color: ''
										}">({{item.plist.length}})</text>	
...
<script setup> 
	...
	async function refresh() {
		if(qxt_loading.value) return 
		await getData()  
	} 
	async function open() { 
		// console.log(1)
		keyword.value = ''
		if(qxt.value.length != 0) return
		await getData()
	} 
	async function getData() {
		qxt_loading.value = true
		await cate.getQxtData()
		qxt_loading.value = false; 
	}
	function searchMenus() {
		...
		menusList.value.forEach(ele => {
			ele.plist.forEach(item => {
				str = item.product.toUpperCase()
				let i = str.indexOf(k)
				if(i > -1) {
					item.pp = [i, k.length]
	...
	function initMenusStatus() { 
		let arr = uni.$u.deepClone(qxt.value);
		console.log(arr)
		arr.forEach(ele => {
			ele.show = true;
			ele.num = 0
			if(ele.plist.length == 0) {
				ele.show = false
			} else { 
				ele.plist.forEach(item => { 
					item.active = false
	...
	function handleChangeShow(item) {
		...
		menusList.value.some((ele, index) => {
			if(ele.exchange == item.exchange) {
				i = index
			}
			return ele.exchange == item.exchange
		})
